...
def find_flights(api_token: str, params: Dict) -> List[Dict]:
    ...
    try:
        ...
        for flight in data['data']:
            ...
            return_date = datetime.fromisoformat(flight.get('return_at', flight['departure_at']))
...
def find_hotels(api_token: str, params: Dict) -> List[Dict]:
    ...
    try:
        ...
        filtered_hotels.sort(key=lambda x: (-x['stars'], x['price_per_night']))
    ...
    for i, p in enumerate(packages[:5], 1):
        with st.expander(f"Вариант {i} | {p['total_price']:,.2f} RUB".replace(',', ' ')):
            ...
            with cols[0]:
                ...
                st.caption(f"Пересадки: {p['flight']['transfers']}")
                
            with cols[1]:
                st.metric("🏨 Отель", f"{p['hotel']['price_per_night']} RUB/ночь")
            ...
            with cols[2]:
                ...
                st.caption(f"🗓 {p['flight']['departure_at'].strftime('%d.%m.%Y')} → {p['flight']['return_at'].strftime('%d.%m.%Y')}")
...
def main():
    ...
    with st.form("search_form"):
        ...
        with cols_filters[1]:
            ...
            min_stars = st.selectbox("Звезды отеля", [0, 2, 3, 4, 5], index=1)
        with cols_filters[2]:
    ...
    if submitted:
        params = {
            ...
            'dates': (departure.strftime('%Y-%m-%d'), return_date.strftime('%Y-%m-%d')),
